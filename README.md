...
├── outputs_a_crf|bilstm/  # 实验输出（结果表、指标、每折模型）
├── outputs_b*/outputs_c/  # B、C 组实验输出
├── outputs_d/             # D 组预训练与下游微调输出
├── outputs_e/             # E 组外部知识实验输出
├── outputs_f/             # F 组实验输出
├── outputs_pause/         # 早期实验输出
├── tests/                 # 测试
└── log.md                 # 详细的实验日志（语料统计、全部结果表）
```
